...
public class ExportPlantUML : Export {
  ...
  private string export_node( Node node, int depth ) {
    ...
    for( int i=0; i<children.length; i++ ) {
      ...
      if( child != null ) {
        retval += export_node( child, (depth + 1) );
      }
    }
  ...
  private void import_node( MindMap map, MatchInfo matches, NodeSide side, ref Node? last_node ) {
    ...
    switch( depth ) {
      case 1 :
        last_node = (Node)map.model.create_root_node( text );
        break;
      case 2 :
        if( last_node != null ) {
          last_node = (Node)map.model.create_main_node( last_node.get_root(), side, text );
        }
        break;
      default :
        if( last_node != null ) {
          ...
          if( (last_depth + 1) == depth ) {
            last_node = (Node)map.model.create_child_node( last_node, text );
          } else if( last_depth == depth ) {
            last_node = (Node)map.model.create_sibling_node( last_node, true, text );
          } else if( last_depth > depth ) {
            for( int i=0; i<(last_depth - depth); i++ ) {
              last_node = (Node)last_node.parent;
            }
            last_node = (Node)map.model.create_sibling_node( last_node, true, text );
          }
        }
