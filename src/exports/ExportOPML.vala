...
public class ExportOPML : Export {
  ...
  public override bool export( string fname, MindMap map ) {
    ...
    opml->add_child( new Xml.Node.comment( _( "Generated by Minder" ) + " " + Minder.static_version ) );
      ...
      if( it->type == Xml.ElementType.ELEMENT_NODE ) {
        if( it->name == "outline") {
          ...
          if (map.get_nodes().length == 0) {
            root.posx = (map.canvas.get_width()  / 2) - 30;
            root.posy = (map.canvas.get_height() / 2) - 10;
          } else {
