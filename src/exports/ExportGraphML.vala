...
public class ExportGraphML : Object {
  ...
  public static bool export( string fname, DrawArea da ) {
    ...
    Xml.Ns*   yns  = new Xml.Ns( root, "http://graphml.graphdrawing.org/xmlns", "y" );
    export_keys( root );
  ...
  private static Xml.Node* export_node_shape( Node node, Theme theme, Xml.Ns* yns ) {
    ...
    geometry->new_prop( "height", node.height.to_string() );
    geometry->new_prop( "width", node.width.to_string() );
    geometry->new_prop( "x", node.posx.to_string() );
    geometry->new_prop( "y", node.posy.to_string() );
    shape->add_child( geometry );

    Xml.Node* fill = new Xml.Node( yns, "Fill" );
    fill->new_prop( "color", Utils.color_from_rgba( node.link_color ) );
    fill->new_prop( "transparent", node.style.node_fill ? "false" : "true" );
    ...
    lbl->new_prop( "fontSize",   node.style.node_font.get_size().to_string() );
    lbl->new_prop( "fontStyle", "plain" );
    lbl->new_prop( "hasBackgroundColor", "false" );
    lbl->new_prop( "hasLineColor", "false" );
    lbl->new_prop( "textColor", Utils.color_from_rgba( node.is_root() ? theme.root_foreground : theme.background ) );
    lbl->new_prop( "visible", node.folded ? "false" : "true" );
    lbl->new_prop( "xml:space", "preserve" );
    lbl->add_content( node.name.text );
    shape->add_child( lbl );

    Xml.Node* s = new Xml.Node( yns, "Shape" );
    s->new_prop( "type", "rectangle" );
    shape->add_child( s );
  ...
  private static Xml.Node* export_link( Node node, Xml.Ns* yns ) {
    ...
    d10->add_child( export_node_lineedge( node, yns ) );
    e->add_child( d10 );
