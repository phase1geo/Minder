...
public class ExportFreemind : Export {
  ...
  private Xml.Node* export_map( MindMap map ) {
    ...
    xmap->add_child( new Xml.Node.comment( _( "Generated by Minder" ) + " " + Minder.static_version ) );
    var nodes = map.get_nodes();
    for( int i=0; i<nodes.length; i++ ) {
      xmap->add_child( export_node( (Node)nodes.index( i ), map ) );
    }
    return( xmap );
  }
  ...
  private void import_map( MindMap map, Xml.Node* n ) {
    ...
    for( int i=0; i<to_nodes.length; i++ ) {
      if( id_map.has_key( to_nodes.index( i ) ) ) {
        var to_node = (map.model.get_node( map.get_nodes(), id_map.get( to_nodes.index( i ) ) ) as Node);
        if( to_node != null ) {
